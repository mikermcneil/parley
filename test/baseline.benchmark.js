...
describe('baseline.benchmark.js', function() {
  ...
  //  baseline.benchmark.js
  //    parley(handleExec)
  // • parley(handleExec)#0 x 23,432 ops/sec ±2.95% (79 runs sampled)
  //      ✓ should be performant enough (using benchSync())
  //    parley(handleExec).exec(cb)
  // • parley(handleExec).exec(cb)#0 x 22,959 ops/sec ±3.83% (78 runs sampled)
  //      ✓ should be performant enough (using benchSync())
  // • parley(handleExec).exec(cb)#0 x 19,548 ops/sec ±2.04% (72 runs sampled)
  //      ✓ should be performant enough (using bench())
  // • parley(handleExec).exec(cb) + artificial setImmediate in handler#0 x 19,135 ops/sec ±1.73% (73 runs sampled)
  //      ✓ should be performant enough (using bench())
  // • parley(handleExec).exec(cb) + artificial setImmediate wrapped around the whole thing#0 x 18,686 ops/sec ±6.00% (71 runs sampled)
  //      ✓ should be performant enough (using bench())
  // ================================================================================================================
  ...
  describe('parley(handleExec).exec(cb)', function(){
    it('should be performant enough (using benchSync())', function (){
      benchSync('parley(handleExec).exec(cb)', [
    ...
    it('should be performant enough (using bench())', function (done){
      bench('parley(handleExec).exec(cb)', [
    ...
    it('should be performant enough (using bench())', function (done){
      bench('parley(handleExec).exec(cb) + artificial setImmediate in handler', [
    ...
    it('should be performant enough (using bench())', function (done){
      bench('parley(handleExec).exec(cb) + artificial setImmediate wrapped around the whole thing', [

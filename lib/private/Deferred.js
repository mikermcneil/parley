      ...
      if (self._userlandAfterExecLCs && arguments[0]) {
        ...
        if (matchingUserlandLC) {
          ...
          // > whatever the intercept handler returned to be our new Error.
            ...
            // Normally, only Error instances are allowed as errors.
            // But for this special case, where the original Error value
            // is being overridden through the use of `.intercept()`, we
            // make a special exception.
            //
            // > Note: This is useful for throwing special signals-- e.g. from
            // > the inside of an actions2 action or a helper in Sails, a fn in
            // > a machinepack, or a commandline script.  If ever we find ourselves
            // > wanting to revert this decision, the old code that used to verify
            // > this was removed in parley@376208fd1c0ab70e7a6b9c4ecfa563ec0d77a3a8.
...
Deferred.prototype.toPromise = function (){
  ...
  var getPromise = bluebird.promisify(function _execShim(){
    // Because of bluebird's "maybeWrapAsError" implementation, we can't send
    // non-errors through to it.  Most of the time, this is fine.
    //
    // But while bluebird's insurance here is admiral, there are some times
    // where this is not welcome -- such as when trying to throw a string
    // (within a machine fn, for example).
    //
    // Here's the relevant bit:
    // https://github.com/petkaantonov/bluebird/blob/e8d8525a0517280d11d6c77ae6b61df86419232b/src/promisify.js#L182-L184
    //
    // So, to work around this, we use this shim function to intercept any
    // non-errors and convert them automatically.
    this.exec(function(){

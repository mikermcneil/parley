...
var customizeOmenOrBuildNewError = require('./customize-omen-or-build-new-error');
...
Deferred.prototype.exec = function(_cb, handleUncaughtException){
  ...
  if (_.isUndefined(_cb)) {
    throw customizeOmenOrBuildNewError(
      'UsageError',
  ...
  if (self._hasBegunExecuting) {
    console.warn(
      ...
      'Stack trace:\n'+
      '```\n'+
      customizeOmenOrBuildNewError('Error','', self._omen).stack.replace(/^.+\n/, '')+
      '```\n'
    );
    return;
  }//-•

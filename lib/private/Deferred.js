...
Deferred.prototype.exec = function(_cb, handleUncaughtException){
  ...
  // Define an inline function that we'll trigger momentarily.
  // > This is necessary to provide an escape hatch from the `try` block that surrounds
  // > the call we'll make to `self._handleExec()` below.)
  var afterHandleExec = function (err, result){
    if (err) {
  ...
  try {
    ...
    self._handleExec(function (err, result){
      ...
      // Trigger the inline function we defined above.
      // (Note that this is _not_ protected by the `try` block.)
      //
      // ----AKKHGHGGGH sorry, I am totally wrong!-----
      // ----AKKHGHGGGH sorry, I am totally wrong!-----
      // ----AKKHGHGGGH sorry, I am totally wrong!-----
      // ----AKKHGHGGGH sorry, I am totally wrong!-----
      // ----AKKHGHGGGH sorry, I am totally wrong!-----
  ...
  if (self._hasFinishedExecuting) {
    // If we've already finished running `handleExec`, then we know it must
    // have been composed purely of blocking (i.e. synchronous) logic.
  }
  else {
    // Otherwise, we know that we're dealing with some non-blocking
    // (i.e. asynchronous) logic.
  }

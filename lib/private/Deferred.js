...
Deferred.prototype.catch = function (_handlerOrEligibilityFilter, _handlerMaybe){
  ...
  if (_.isString(_handlerOrEligibilityFilter)) {
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // TODO: Come back to this -- something about this change causes unhandled rejection issues
    // Example from the machine runner:
    // ```
    // require('./')({ exits:{notFound: {}},fn: (i,x)=>{    require('./')({ exits:{notFound: {}},fn: (i,x)=>{    return x.notFound('k');} })().then(x.success).catch('notFound', x.notFound).catch(x.error);  } })({}).switch({notFound: ()=>{console.log('got notFound'); }, error: (err)=>{console.log('!',err);} })
    // ```
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  }
...
// FUTURE: Maybe add a special `.handle()` which is almost exactly like `.catch()`,
// except that the return value of the provided function is completely ignored.
// If it throws, then the promise is rejected.
//

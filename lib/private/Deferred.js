...
Deferred.prototype.exec = function(_cb, handleUncaughtException){
  ...
  // > Note that, if the executioner is an ES8 async function, then we also attach a handler to
  // > `.catch()` its return value (which will be a promise) in order to react to unhandled promise
  // > rejections in the same way we handle uncaught exceptions that are thrown synchronously.
  ...
  if (self._handleExec.constructor.name === 'AsyncFunction') {
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      // FUTURE: benchmark this and, if tangible enough, provide some mechanism for passing in this
      // information so that it can be predetermined (e.g. at build-time).
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      var promise = self._handleExec(/* TODO */)
      promise.catch(function(e) { /* TODO */ });
    }
    else {
      self._handleExec(/* TODO */);
    }

 ...
 */
Deferred.prototype.intercept = function(){
 ...
 *
 */
Deferred.prototype.tolerate = function(){
  throw new Error('TODO');
};
  ...
  if (!isFinishedExecuting) {
    throw flaverr({
      ...
      message:
        ...
        'it is not actually synchronous.  Instead, its implementation is asynchronous--\n'+
        'which means it is not compatible.  Rather than `.now()`, please use `await` or `.exec()`.'
    }, this._omen);
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // FUTURE: use some kind of mechanism to indicate
    ...
    // i.e. because we've already sent back a proper error, and the warnings
    // just muddy the waters, so to speak.
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  }//•

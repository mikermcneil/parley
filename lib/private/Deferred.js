...
function proceedToInterceptsAndChecks (errCbArg, resultCbArg, extraCbArgs, self, _cb, handleUncaughtException) {
  ...
  if (!(self._userlandAfterExecLCs && errCbArg)) {
    ...
    (function(proceed){
      ...
      else if (matchingUserlandLC.handler === undefined && matchingUserlandLC.type === 'retry') {
        matchingUserlandLC.handler = function(err){
          self._errorsThatCausedRetries.push(err);
          return new Promise(function(resolve, reject) {
            // TODO: implement asynchronous whilst here instead of a one-time retry
            // for (var i=0; i<self._retryDelaySeries.length; i++) {
            //   // this._errorsThatCausedRetries = [];
            //   // this._retryDelaySeries = [100, 200, 400];
            //   // TODO
            // }//∞
            var π0 = new Deferred(function(proceed) {
              // TODO
              proceed();
            })
            ...
            // Apply timeout and omen, if applicable.
            if (self._timeout) {
              π0._timeout = self._timeout;
            }//ﬁ
            if (self._omen) {
              π0._omen = self._omen;
            }//ﬁ
      ...
      var isSynchronousHandler = (
        matchingUserlandLC.handler.constructor.name !== 'AsyncFunction' &&
        matchingUserlandLC.type !== 'retry'
        // ^^Note that we cannot safely make the "retry" handler an `async function`
        // and also maintain backwards compatibility with older versions of Node.js.
        // So instead, we make a special exception for it here.
      );

      // Run userland LC.
      self._hasStartedButNotFinishedAfterExecLC = true;
      if (isSynchronousHandler) {
      ...
      if (matchingUserlandLC.type === 'tolerate' || matchingUserlandLC.type === 'retry') {
        errCbArg = undefined;

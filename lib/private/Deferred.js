      ...
      if (self._userlandAfterExecLCs && arguments[0]) {
        ...
        if (matchingUserlandLC) {
          ...
          try {
            ...
            // TODO: finish this
            // If this is an .intercept() handler and, instead of an Error instance,
            // a string or dictionary was thrown, then it's likely the handler was
            // attempting to send a special signal.  In this case, we'll use a slightly
            // different error message.
            var didThrowDictionary = (
              !_.isError(err) && _.isObject(err) && !_.isArray(err) && !_.isFunction(err)
            );
            if (matchingUserlandLC.type === 'intercept' && (didThrowDictionary || _.isString(err))) {
              throw flaverr({
                name:
                  'UsageError',
                message:
                  'Caught unexpected '+(didThrowDictionary?'dictionary':'string')+' thrown from the `.intercept()` handler function.  '+
                  'Instead of throwing from inside of the `.intercept()` handler function, please return the signal you would like '+
                  'to be thrown instead, and it will be thrown automatically.',
                raw:
                  err
              }, self._omen);
            }
          ...
          // > whatever the intercept handler returned to be our new Error.
            ...
            // var didReturnDictionary = (
            //   !_.isError(resultFromHandler) &&
            //   _.isObject(resultFromHandler) &&
            //   !_.isArray(resultFromHandler) &&
            //   !_.isFunction(resultFromHandler)
            // );

            // var didReturnSpecialExitSignal = (
            //   _.isString(resultFromHandler) ||
            //   (
            //     didReturnDictionary &&
            //     _.difference(Object.keys(resultFromHandler), ['exit', 'output']).length === 0 &&
            //     (
            //       resultFromHandler.exit !== undefined &&
            //       _.isString(resultFromHandler.exit) &&
            //       resultFromHandler.exit !== ''
            //     ) &&
            //     (
            //       resultFromHandler.output !== undefined ?
            //       resultFromHandler.output
            //       :
            ...
            if (_.isError(resultFromHandler)){
              ...
              throw flaverr({
                ...
                message:
                  ...
                  'Expected an Error instance but instead, got: '+
                  util.inspect(resultFromHandler, {depth: 5})
              }, self._omen);
            }//ﬁ

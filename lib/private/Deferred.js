      ...
      if (self._userlandAfterExecLCs && arguments[0]) {
        ...
        if (matchingUserlandLC) {
          ...
          // > whatever the intercept handler returned to be our new Error.
            ...
            var interceptError = flaverr.parseError(resultFromHandler) || resultFromHandler;
            if (!_.isError(interceptError)) {
              interceptError = flaverr({
                name: 'Envelope',
                message: _.isString(resultFromHandler) ? resultFromHandler : util.inspect(resultFromHandler, {depth: 5}),
                code: 'E_NON_ERROR',
                traceRef: self,
                raw: resultFromHandler
              });
            }//ﬁ

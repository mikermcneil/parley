...
Deferred.prototype.exec = function(_cb, handleUncaughtException){
  ...
  // Otherwise, we use the regular approach, which just adds some additional checks:
  else {
    // Conditionally (if multiple ticks have already elapsed at the time it's being called) wrap
    // the cb from userland in a function that injects a try/catch.  If an unhandled error of
    // any kind is thrown from the userland cb, our wrapper uses a special Envelope to bust out
    // of the `try` block, ensuring that the unhandled exception is thrown up to userland.
    //
    // > NOTE:
    ...
    cb = function _tryToRunCb(/*…*/) {
      ...
      // Like before, note that we don't use .slice(). (See above for reasoning.)
      if (self._hasAlreadyTakenMultipleTicks) {
        return _cb.apply(undefined, arguments);
      }//•
      else {
        try {
  ...
  // Use `self._hasAlreadyTakenMultipleTicks` to track whether or not this logic is asynchronous.
  if (self._hasFinishedExecuting) {
    ...
    self._hasAlreadyTakenMultipleTicks = false;
  }
  else {
    ...
    self._hasAlreadyTakenMultipleTicks = true;
  }

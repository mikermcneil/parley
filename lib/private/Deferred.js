...
function Deferred(handleExec){
  ...
  // flags.)  This is really just as a nicety.  The reason it was so slow before
  // was likely due to the fact that it got run on every invocation, whereas now
  // it could be implemented so that it runs exactly once per process.
  //
  // Why bother?  Mainly because it makes Deferred instances nicer to look at,
  // but also because that way, `._handleExec` looks a lot less tantalizing from
  // userland.
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

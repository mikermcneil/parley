...
Deferred.prototype.exec = function(_cb, handleUncaughtException){
  ...
  // Otherwise, just use `_cb` as-is.
    ...
    // "Also, here are the available keys on `self` at this point:\n```\n_handleExec,meta,usingConnection,where,limit,skip,paginate,sort,sum,avg,min,max,groupBy,select,omit,populateAll,populate,_WLModel,_wlQueryInfo,_hasBegunExecuting,_hasFinishedExecuting,_hasTakenMultipleTicks"
    //
    // Some relevant links:
  ...
  try {
    ...
    self._handleExec(function (err, result) {
      ...
      if (err) {
        ...
        if (self._hasTakenMultipleTicks) {
          cb(err);
        } else {
          try { cb(err); }
            ...
            setImmediate(function(){
              // Crash the process.
              throw unexpectedErrorFromCallback;
            });//_∏_
          }
        }//ﬁ
  ...
  // Use `self._hasTakenMultipleTicks` to track whether or not this logic is asynchronous.
  if (self._hasFinishedExecuting) {
    ...
    self._hasTakenMultipleTicks = false;
  }
  else {
    ...
    self._hasTakenMultipleTicks = true;
  }

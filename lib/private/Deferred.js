  ...
  try {
    ...
    self._handleExec(function (err, result) {
      ...
      if (arguments.length > 2) {
        return proceedToInterceptsAndChecks(err, result, Array.prototype.slice.call(arguments, 2), self, _cb, handleUncaughtException);
      }
      ...
      // ^^TODO: move this elsehwere
        ...
        return proceedToInterceptsAndChecks(undefined, undefined, undefined, self, _cb, handleUncaughtException);
      }
      // Otherwise, there's a result, so send it back.
      else {
        return proceedToInterceptsAndChecks(undefined, result, undefined, self, _cb, handleUncaughtException);
      }
...
// just a potentially-more-efficient alternative to a series of self-calling functions.
...
function proceedToInterceptsAndChecks (errCbArg, resultCbArg, extraCbArgs, self, _cb, handleUncaughtException) {
  ...
  // // > Note that we don't use .slice() on the `arguments` keyword unless absolutely
  // // > necessary -- this is for performance reasons.
  // // > (see https://github.com/petkaantonov/bluebird/wiki/Optimization-killers#what-is-safe-arguments-usage)
  // var errCbArg;
  // var resultCbArg;
  // var extraCbArgs;

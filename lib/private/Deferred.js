...
Deferred.prototype.exec = function(_cb, handleUncaughtException){
  ...
  try {
    self._handleExec(function (err, result) {
      ...
      if (err) {
        ...
        // So we have to be a bit relaxed here.
        var isBluebirdErrorish = _.isObject(err) && _.isError(err.cause);
        // console.error();
        // console.error('What is it??:');
        // console.error('-----------------------------------');
        // console.error('typeof err', typeof err);
        // console.error('_.isError(err)', _.isError(err));
        // console.error('_.isObject(err)', _.isObject(err));
        // console.error('_.isFunction(err)', _.isFunction(err));
        // console.error('Object.keys(err)', Object.keys(err));
        // console.error('_.keys(err)', _.keys(err));
        // console.error('err.constructor.name', err.constructor.name);
        // console.error('err.name', err.name);
        // console.error('err.message', err.message);
        // // console.error('err.stack', err.stack);
        // console.error();
        // console.error('err.raw', err.raw);
        // console.error();
        // console.error('_.isError(err.cause)', _.isError(err.cause));
        ...
        // console.error('-----------------------------------');
        // console.error();
        // console.log('bluebird errorish?', isBluebirdErrorish);
        if (_.isError(err)) { /* ok */ }
        else if (isBluebirdErrorish) { err = err.cause; }
  ...
  // Handle errors thrown synchronously by the `_handleExec` implementation:
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // ^ FUTURE: Better error message for this third case, and
    // use omens for both of the `new Error()` calls, if possible.
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

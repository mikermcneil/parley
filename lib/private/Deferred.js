 ...
 */
Deferred.prototype.catch = function (firstArg, secondArg){
  var promise = this.toPromise();

  // If first argument (`firstArg`) is a string, convert it automatically to `{code: firstArg}`
  //
  // > This is for consistency between bluebird's "filtered catch" and "flaverr" / the machine runner.
  // > It also allows for more concise exception handling code.
  // >  • http://bluebirdjs.com/docs/api/catch.html
  if (_.isString(firstArg)){
    firstArg = { code: firstArg };
  }

  return promise.catch.apply(promise, [firstArg, secondArg]);
};

 ...
 *
 * @param {Boolean?|Error?} omen
 *        An optional omen to use for improving the stack trace, or `true` to tell parley
 *        to build an omen itself.
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * @returns {Deferred}
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
module.exports = function parley(handleExec, explicitCbMaybe, customMethods, timeout, omen){
  ...
  // But first, depending on how parley was called, we may need to build an omen for
  // potential use in the asynchronous callback below.
  // > This improves the quality of the Error stack -- only for the deferred object
  // > though, not explicit cb usage.
  if (omen) {
    if (omen === true) {
      var omen = buildOmen(omen);
    }
    else if (_.isError(omen)) {

    }
    else { throw new Error('Consistency violation: `omen` argument to parley should be either `true` or a pre-existing omen (i.e. an Error instance).  But instead, got: '+util.inspect(omen, {depth:2})+''); }
  }

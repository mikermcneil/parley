 ...
 *
 * @param {Function?} interceptAfterExec
...
module.exports = function parley(handleExec, explicitCbMaybe, customMethods, timeout, omen, interceptAfterExec){
  ...
  // If the executioner is an ES8 async function, then fail by throwing an error.
  // (This could technically be supported now, but performance reasons, we avoid it.)
  // See the implementation in the machine runner (https://npmjs.com/package/machine)
  // and in Sails core for some examples of how to go about implementing this in your
  // own userland code.  (In particular, the machine runner impl will be useful.)
  if (handleExec.constructor.name === 'AsyncFunction') {
    throw new Error('Consistency violation: For performance reasons, the first argument to parley() should be a plain-jane function-- not an ES8 async function.  You can easily make this work in your own code (see the machine runner for an example).  But the relevant code is too heavy and this code path is too hot for it to be included here.  To resolve this, please call parley with a normal (non-async) function instead.');
  }
  ...
  if (customMethods) {
    ...
    for (var methodName in customMethods) {
      ...
      if (
        ...
        methodName === '_timeout' ||
        methodName === '_omen' ||
        methodName === '_interceptAfterExec' ||
  ...
  //  │───  │├┤   ├┬┘├┤ │  ├┤ └┐┌┘├─┤│││ │   ───│
  //  └─    ┴└    ┴└─└─┘┴─┘└─┘ └┘ ┴ ┴┘└┘ ┴     ─┘
  if (interceptAfterExec) {
    if (!_.isFunction(interceptAfterExec)) { throw new Error('Consistency violation: If provided, `interceptAfterExec` argument to parley should be a handler function (see parley README for more information or visit https://sailsjs.com/support for help).  But instead of that function, got: '+util.inspect(interceptAfterExec, {depth:5})+''); }
    π._interceptAfterExec = interceptAfterExec;
  }

...
module.exports = function parley(handleExecOrOpts, explicitCbMaybe){
  ...
  // Determine whether or not to verify usage.
  // (currently, this is just disabled all the time)
  var doCheckUsage = false;
  // var doCheckUsage = false || process.env.NODE_ENV !== 'production';
  ...
  if (doCheckUsage) {
    if (_.isUndefined(handleExecOrOpts)){
      throw new Error('Must specify a first argument when calling parley() -- please provide a `handleExec` function or a dictionary of options');
    }
  }
  ...
  if (_.isFunction(handleExecOrOpts)) {
    ...
    if (doCheckUsage) {
      if (_.isUndefined(handleExecOrOpts.handleExec)){
        throw new Error('If specifying first argument as a dictionary of options, then a valid `handleExec` function must be specified as an option.');
      }
    }
    ...
    if (handleExecOrOpts.codeName) {
      if (doCheckUsage) {
        if (!_.isUndefined(handleExecOrOpts.codeName) && !_.isString(handleExecOrOpts.codeName)) {
          throw new Error('If specified, `codeName` option must be a string.  But instead, got: '+util.inspect(handleExecOrOpts.codeName, {depth:5})+'');
        }
        if (codeName === ''){
          throw new Error('Specified codeName (empty string: \'\') is not valid.');
        }
      }
  ...
  if (doCheckUsage) {
    ...
    if (!_.isFunction(handleExec)) {
      throw new Error('Invalid `handleExec` function: '+util.inspect(handleExec, {depth:5})+'');
    }

    // Verify `explicitCbMaybe` (if present)
    if (!_.isUndefined(explicitCbMaybe)) {
      if (!_.isFunction(explicitCbMaybe)) {
        throw new Error('If specified, 2nd argument ("explicitCbMaybe") must be a function.  But instead, got: '+util.inspect(explicitCbMaybe, {depth:5})+'');
      }
    }

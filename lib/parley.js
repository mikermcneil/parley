 ...
 *
 * @param {Function?} interceptBuiltInError
 *        An optional, synchronous handler function for intercepting built-in errors like
 *        timeouts, double-invocation, and unhandled throwing, on the way out.  These are
 *        errors that would be otherwise difficult to capture (at least without reinventing
 *        the wheel).  This is useful for saving some work in places like `machine-as-action`
 *        and makes it easier to include nicer, more custom error msgs without making too
 *        much of a mess.
 *        TODO: finish this or get rid of it (might not matter since .exec() is ubiquitous)
 *        **UPDATE: Nevermind- for posterity, the reason this is important is because we
 *        sometimes want to change the behavior of .exec() without necessarily calling it.
 *        This COULD, however, be a generic lifecycle callback, which is probably a bit
 *        cleaner/easier to implement. **
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * @returns {Deferred}
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
module.exports = function parley(handleExec, explicitCbMaybe, customMethods, timeout, omen, interceptBuiltInError){
  ...
  if (customMethods) {
    ...
    for (var methodName in customMethods) {
      ...
      if (
        ...
        methodName === '_timeout' ||
        methodName === '_omen' ||
        methodName === '_interceptBuiltInError' ||
  ...
  //  └─    ┴└    ┴└─└─┘┴─┘└─┘ └┘ ┴ ┴┘└┘ ┴     ─┘
  if (timeout) {
    if (!_.isNumber(timeout)) { throw new Error('Consistency violation: `timeout` argument to parley should be a number (i.e. max # of milliseconds to wait before giving up).  But instead, got: '+util.inspect(timeout, {depth:2})+''); }
    π._timeout = timeout;
  }
  ...
  //  └─    ┴└    ┴└─└─┘┴─┘└─┘ └┘ ┴ ┴┘└┘ ┴     ─┘
  if (omen) {
    if (!_.isError(omen)) { throw new Error('Consistency violation: `omen` argument to parley should be a pre-existing omen (i.e. an Error instance).  But instead, got: '+util.inspect(omen, {depth:2})+''); }
    π._omen = omen;
  }
  ...
  //  │───  │├┤   ├┬┘├┤ │  ├┤ └┐┌┘├─┤│││ │   ───│                                 
  //  └─    ┴└    ┴└─└─┘┴─┘└─┘ └┘ ┴ ┴┘└┘ ┴     ─┘                                 
  if (interceptBuiltInError) {
    if (!_.isFunction(interceptBuiltInError)) { throw new Error('Consistency violation: `interceptBuiltInError` argument to parley should be a handler function that receives the raw (mutable) Error instance, then returns either a new Error or the original Error after potentially performing modifications.  But instead of that function, got: '+util.inspect(interceptBuiltInError, {depth:5})+''); }
    π._interceptBuiltInError = interceptBuiltInError;
  }
